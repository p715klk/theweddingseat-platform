/**
 * 生成 PocketBase hooks（每個 routerAdd handler 獨立 eval，helper 必須 inline）
 * 用法：node scripts/build-tws-hooks.mjs
 */
...
const outPath = resolve(root, 'pocketbase/pb_hooks/tws_routes.pb.js');

/** PocketBase 新舊版 DB 查詢（每個 handler 都要 inline） */
const QUERY_LAYER = `
...
/** helper 必須 inline 到每個 POST route */
const HELPERS = `
...
`,
  ...
  {
    path: '/tws/update-member-profile',
    body: `
...
let output = `/// <reference path="../types.d.ts" />\n`;
output += `// AUTO-GENERATED by scripts/build-tws-hooks.mjs — 唔好手改\n`;
output += `// 部署 NAS：複製 tws_routes.pb.js + tws.pb.js 去 pb_data/pb_hooks/\n\n`;

output += `var twsUserAuthMiddleware = (function() {
  try {
...
for (const route of ROUTES) {
  output += `routerAdd("POST", "${route.path}", function(e) {${HELPERS}${route.body}\n}, twsUserAuthMiddleware);\n\n`;
}
