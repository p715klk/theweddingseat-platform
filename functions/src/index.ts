...
function buildDefaultStarterGuestsPayload() {
  ...
  return {
    wedding_guests: {
      "1": [
        { id: mkId(), name: "Bride", side: "女方", group: "家人", sort: 1 },
        { id: mkId(), name: "Groom", side: "男方", group: "家人", sort: 2 },
      ],
    },
    unassigned_guests: [] as any[],
    guest_status: {} as Record<string, any>,
  };
}
...
function normalizeMemberRole(val: unknown): MemberRole {
  if (val === true) return "admin";
  if (val === "admin" || val === "reception") return val;
  return "";
}

async function isTenantOwner(tenantId: string, uid: string) {
  const db = getDatabase();
...
async function uidHasOtherMemberships(uid: string, excludeTenantId: string) {
  const db = getDatabase();
  ...
  for (const tenantIdRaw of Object.values(slugs)) {
    ...
    if (memberSnap.exists() && normalizeMemberRole(memberSnap.val())) return true;
  }
  return false;
}

async function uidIsOwnerOfAnyTenant(uid: string, excludeTenantId?: string) {
  const db = getDatabase();
  const slugsSnap = await db.ref("slugs").get();
  const slugs = slugsSnap.val() || {};
  for (const tenantIdRaw of Object.values(slugs)) {
    const tenantId = String(tenantIdRaw);
    if (excludeTenantId && tenantId === excludeTenantId) continue;
    const ownerSnap = await db.ref(`tenants/${tenantId}/meta/owner_uid`).get();
    if (ownerSnap.exists() && ownerSnap.val() === uid) return true;
  }
  return false;
}

async function shouldDeleteAuth(uid: string, excludeTenantId: string) {
  if (await isPlatformAdmin(uid)) return false;
  if (await uidIsOwnerOfAnyTenant(uid, excludeTenantId)) return false;
  if (await uidHasOtherMemberships(uid, excludeTenantId)) return false;
  return true;
}

async function collectTenantMembershipCleanup(uid: string) {
  const db = getDatabase();
...
export const removeTenantMember = onCall(
  { region: "asia-southeast1" },
  async (req): Promise<{ tenantId: string; uid: string; authDeleted: boolean }> => {
    if (!req.auth?.uid) {
      throw new HttpsError("unauthenticated", "需要登入");
    }
