...
| **User Backend** | Owner / Project admin（統籌） | `/p/{slug}/admin` | 賓客、排位、標籤；**只有 Owner 可管理用戶** |
| **點名頁** | user（現場接待） | `/p/{slug}` | 要登入；點名、取消賓客、現場加座；**不可入後台** |
...
#### 權限模型（RBAC）— 4 層角色（2026-06-19 釐清）

> 由上至下繼承：**super admin ⊃ Owner ⊃ Project admin ⊃ user**。

| 角色 | 可去邊個路由 | 主要能力 | Firebase / RTDB 對應 |
|------|--------------|----------|---------------------|
| **super admin** | `/super/*` + 任何 `/p/{slug}/*` | 平台同所有 project 後台；開/停 tenant、改 plan/slug、加/移 Owner 等；**擁有 Owner 一切權限** | `platform_admins/{uid} = true` |
| **Owner** | 自己 project：`/p/{slug}/admin`、`/p/{slug}/seating`、`/p/{slug}` | Project 最高權限；**可為自己 project 新增/移除用戶**；擁有 Project admin 一切權限 | `tenants/{id}/meta/owner_uid = <uid>`（`members` 記錄可選，前端會當 admin 處理） |
| **Project admin** | 自己 project：`/p/{slug}/admin`、`/p/{slug}/seating`、`/p/{slug}` | 後台賓客、排位、CSV 等；**不可新增/移除用戶**；擁有 user 一切權限 | `tenants/{id}/members/{uid} = 'admin'`（legacy：`true` 等同 admin） |
| **user** | 自己 project：`/p/{slug}` **only** | 點名頁：改 `guest_status`（點名／取消）；**現場加座**（受限欄位新增 `wedding_guests`）；**不可入後台** | `tenants/{id}/members/{uid} = 'reception'`（UI 顯示「現場接待」；之後可改名 `'user'`） |

**登入要求（規劃中）：** 之後所有 project 會設 `requireLogin = true`；點名頁 `/p/{slug}` 亦要登入，未登入不可操作。

**程式碼對照：**

| 文件角色 | `useTenantAccess` | `AdminSettingsDialog` 建立時選項 |
|----------|-------------------|----------------------------------|
| Owner | `memberRole = 'owner'`（或 `owner_uid` 匹配） | —（provision 時由 Super Admin / `createTenant` 指定） |
| Project admin | `memberRole = 'admin'` | 「後台管理員」 |
| user | `memberRole = 'reception'` | 「現場接待」 |

#### Owner 權限核查（2026-06-19）

| 能力 | Owner | Project admin | user | 實作位置 |
|------|:-----:|:-------------:|:----:|----------|
...
| 寫 `table_settings` / `floor_layout` | ✅ | ✅ | ❌ | Rules `features.seating` |
...
|------|----------------------------------------|
| `guest_status/*` | Owner / Project admin / **user**；且 `features.checkin !== false` |
| `wedding_guests`（完整）、`unassigned_guests`、`meta_label_columns` | Owner / Project admin；且 `features.guestlist !== false` |
| `wedding_guests/{table}/{idx}`（現場加座） | 上述 **或 user**（`reception`）且 `$idx` 為新增（`!data.exists()`） |
| `table_settings`、`floor_layout` | Owner / Project admin；且 `features.seating !== false` |
| `members`、`user_profiles`（他人） | **Owner only**（super admin 亦可） |
...
唔止「有 login」，要定 **角色（RBAC）** — 四層繼承（詳見 §0.5）：

| 角色 | 權限 |
|------|------|
| `super_admin` | 可入 `/super`；可管理所有 tenants；**擁有 Owner 一切權限** |
| `owner` | 可入自己 `/p/{slug}/admin`、`/seating`、`/p/{slug}`；**可新增/移除 project 用戶**；完整後台 |
| `project_admin` | 可入自己 `/p/{slug}/admin`、`/seating`、`/p/{slug}`；賓客/排位/CSV；**不可管理用戶** |
| `user` | 只可用 `/p/{slug}`；可改 `guest_status`（點名、取消）；可現場加座（受限寫入）；**不可入後台** |

> 規劃：全部 project `requireLogin = true` 後，點名頁亦需登入；QR 分開帶位（user）同統籌（Project admin / Owner）。
